...
void Noritake_GUU100::init (const void *fontPtr, uint8_t hofs, uint8_t vofs)
{
	...
	_fontStack = 0; // init push/popFont() stack pointer
...
// this is for the USER - do not use in this driver
void Noritake_GUU100::pushFont (void)
{
	pushFont ((const void *)(0), 0, 0);
}

// this is for the USER - do not use in this driver
void Noritake_GUU100::pushFont (const void *fontPtr, uint8_t hofs, uint8_t vofs)
{
	if (_fontStack < sizeOF(_fontSave)) {

		_fontSave[_fontStack] = getFont();
		_hofsSave[_fontStack] = _hofs;
		_vofsSave[_fontStack] = _vofs;
...
void Noritake_GUU100::popFont (void)
{
	...
	setFont (_fontSave[_fontStack], _hofsSave[_fontStack], _vofsSave[_fontStack]);
}
...
uint8_t Noritake_GUU100::_checkXY (uint8_t x, uint8_t y)
{
